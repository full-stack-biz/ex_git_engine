defmodule ExGitEngine.GitAgent do
  ...
  """
  @spec odb_read(agent, GitOdb.t(), Git.oid(), keyword) ::
          {:ok, {Git.obj_type(), binary}} | {:error, term}
  ...
  def odb_write(agent, odb, data, type, opts \\ []),
    do: exec(agent, {:odb_write, odb, data, type}, opts)

  @doc """
  Returns `true` if the given `oid` exists in `odb`; elsewise returns `false`.
  """
  @spec odb_object_exists?(agent, GitOdb.t(), Git.oid(), keyword) ::
          {:ok, boolean} | {:error, term}
  def odb_object_exists?(agent, odb, oid, opts \\ []),
    do: exec(agent, {:odb_object_exists?, odb, oid}, opts)
  ...
  """
  @spec odb_writepack_append(
          agent,
          GitWritePack.t(),
          binary,
          Git.odb_writepack_progress(),
          keyword
        ) :: {:ok, Git.odb_writepack_progress()} | {:error, term}
  def odb_writepack_append(agent, writepack, data, progress, opts \\ []),
    do: exec(agent, {:odb_writepack_append, writepack, data, progress}, opts)

  @doc """
  Commits the given `writepack` to the ODB.
  """
  @spec odb_writepack_commit(agent, GitWritePack.t(), Git.odb_writepack_progress(), keyword) ::
          {:ok, Git.odb_writepack_progress()} | {:error, term}
  def odb_writepack_commit(agent, writepack, progress, opts \\ []),
    do: exec(agent, {:odb_writepack_commit, writepack, progress}, opts)
  ...
  """
  @spec branch(agent, binary, keyword) ::
          {:ok, GitRef.t() | {GitRef.t(), GitCommit.t()}} | {:error, term}
  ...
  """
  @spec tag(agent, binary, keyword) ::
          {:ok, GitRef.t() | GitTag.t() | {GitRef.t() | GitTag.t(), GitCommit.t()}}
          | {:error, term}
  ...
  """
  @spec reference(agent, binary, keyword) ::
          {:ok, GitRef.t() | {GitRef.t(), GitCommit.t()}} | {:error, term}
  ...
  """
  @spec graph_ahead_behind(agent, Git.oid(), Git.oid(), keyword) ::
          {:ok, {non_neg_integer, non_neg_integer}} | {:error, term}
  def graph_ahead_behind(agent, local, upstream, opts \\ []),
    do: exec(agent, {:graph_ahead_behind, local, upstream}, opts)
  ...
  def commit_committer(agent, commit, opts \\ []),
    do: exec(agent, {:commit_committer, commit}, opts)
  ...
  def commit_timestamp(agent, commit, opts \\ []),
    do: exec(agent, {:commit_timestamp, commit}, opts)
  ...
  def commit_gpg_signature(agent, commit, opts \\ []),
  ...
  def commit_create(agent, author, committer, message, tree_oid, parents_oids, opts \\ []) do
    ...
    exec(
      agent,
      {:commit_create, update_ref, author, committer, message, tree_oid, parents_oids},
      opts
    )
  end
  ...
  """
  @spec tree_entry_by_id(agent, git_revision | GitTree.t(), Git.oid(), keyword) ::
          {:ok, GitTreeEntry.t()} | {:error, term}
  ...
  """
  @spec tree_entry_by_path(agent, git_revision | GitTree.t(), Path.t(), keyword) ::
          {:ok, GitTreeEntry.t() | {GitTreeEntry.t(), GitCommit.t()}} | {:error, term}
  ...
  """
  @spec tree_entries(agent, git_revision | GitTree.t(), keyword) ::
  ...
  def index_add(agent, index, index_entry, opts \\ []),
    do: exec(agent, {:index_add, index, index_entry}, opts)

  @doc """
  Adds an index entry to the given `index`.
  """
  @spec index_add(agent, GitIndex.t(), Git.oid(), Path.t(), non_neg_integer, pos_integer, keyword) ::
          :ok | {:error, term}
  def index_add(agent, index, oid, path, file_size, mode, opts \\ []),
    do: exec(agent, {:index_add, index, oid, path, file_size, mode, opts}, opts)

  @doc """
  Removes an index entry from the given `index`.
  """
  @spec index_remove(agent, GitIndex.t(), Path.t(), keyword) :: :ok | {:error, term}
  def index_remove(agent, index, path, opts \\ []),
    do: exec(agent, {:index_remove, index, path}, opts)
  ...
  def index_remove_dir(agent, index, path, opts \\ []),
    do: exec(agent, {:index_remove_dir, index, path}, opts)

  @doc """
  Reads the given `tree` into the given `index`.
  """
  @spec index_read_tree(agent, GitIndex.t(), GitTree.t(), keyword) :: :ok | {:error, term}
  def index_read_tree(agent, index, tree, opts \\ []),
    do: exec(agent, {:index_read_tree, index, tree}, opts)

  @doc """
  Writes the given `index` as a tree.
  """
  @spec index_write_tree(agent, GitIndex.t(), keyword) :: {:ok, Git.oid()} | {:error, term}
  def index_write_tree(agent, index, opts \\ []),
  ...
  """
  @spec peel(agent, git_revision | GitTreeEntry.t(), keyword) ::
          {:ok, git_object} | {:error, term}
  ...
  """
  @spec transaction(agent, term, (Git.repo() -> {:ok, term} | {:error, term}), keyword) ::
          {:ok, term} | {:error, term}
  ...
  @impl true
  def handle_call(op, {pid, _tag}, {handle, config} = state)
      when elem(op, 0) in [
             :references,
             :references_with,
             :history,
    ...
    {chunk_size, opts} =
      Keyword.pop(elem(op, opts_index), :stream_chunk_size, config.stream_chunk_size)

    case call_stream(handle, put_elem(op, opts_index, opts), chunk_size, pid) do
      {:ok, stream} ->
        {:reply, {:ok, stream}, {handle, collect_result_refs(config, stream, pid)},
         config.idle_timeout}
  ...
  def handle_call(op, {pid, _tag}, {handle, %{cache: cache} = config} = state) do
    case call_cache(handle, op, cache, pid) do
      ...
      {:ok, result} ->
        {:reply, {:ok, result}, {handle, collect_result_refs(config, result, pid)},
         config.idle_timeout}
  ...
  defp exec(agent, op, opts) when is_pid(agent),
    do:
      telemetry(
        :call,
        op,
        fn -> GenServer.call(agent, op, Keyword.get(opts, :timeout, @default_config.timeout)) end,
        %{pid: agent}
      )
  ...
  defp call(handle, {:reference_with, with_target, "refs/" <> _suffix = name, target}) do
    with {:ok, shorthand, :oid, oid} <- Git.reference_lookup(handle, name),
         ...
         {:ok, target} <- fetch_target(ref, with_target, handle),
         do: {:ok, {ref, target}}
  end
  ...
  defp call(handle, {:reference_create, name, type, target, force}),
    do: Git.reference_create(handle, name, type, target, force)
  ...
  defp call(_handle, {:odb_write, %GitOdb{__ref__: odb}, data, type}),
    do: Git.odb_write(odb, data, type)
  ...
  defp call(handle, :odb_writepack) do
    with {:ok, odb} <- Git.repository_get_odb(handle),
         {:ok, writepack} <- Git.odb_get_writepack(odb),
         do: {:ok, resolve_writepack(writepack)}
  end
  ...
  defp call(_handle, {:odb_writepack_commit, %GitWritePack{__ref__: writepack}, progress}),
    do: Git.odb_writepack_commit(writepack, progress)
  ...
  defp call(handle, {:tree_entry_with, with_target, obj, spec}) do
    with {:ok, tree_entry} <- fetch_tree_entry(obj, spec, handle),
         {:ok, target} <- fetch_tree_entry_target(with_target, obj, spec, handle),
         do: {:ok, {tree_entry, target}}
  end
    ...
    with {:ok, tree_entries} <- fetch_tree_entries(rev, handle),
         ...
         do:
           {:ok,
            Stream.transform(
              commits,
              Map.new(tree_entries, &{&1.name, &1}),
              &zip_tree_entries_target(with_target, &1, &2, handle)
            )}
  end
    ...
    with {:ok, root_tree_entry} <- fetch_tree_entry(rev, {:path, path}, handle),
         ...
         do:
           {:ok,
            Stream.transform(
              commits,
              {root_tree_entry, Map.new(tree_entries, &{Path.join(path, &1.name), &1})},
              &zip_tree_entries_target(with_target, &1, &2, handle)
            )}
  end
  ...
  defp call(_handle, {:index_remove, %GitIndex{__ref__: index}, path}),
    do: Git.index_remove(index, path)

  defp call(_handle, {:index_remove_dir, %GitIndex{__ref__: index}, path}),
    do: Git.index_remove_dir(index, path)

  defp call(_handle, {:index_read_tree, %GitIndex{__ref__: index}, %GitTree{__ref__: tree}}),
    do: Git.index_read_tree(index, tree)
  ...
  defp call(_handle, op),
    do: {:error, %GitError{message: "invalid operation #{inspect(op)}", code: -21}}

  defp call_cache(handle, op, cache), do: call_cache(handle, op, cache, self())

  defp call_cache(handle, {:transaction, _name, _cb} = op, cache, pid)
       when not is_tuple(handle) do
    ref = make_ref()
    ...
    telemetry(:transaction_stop, op, %{duration: :os.system_time(:microsecond) - event_time}, %{
      ref: ref,
      pid: pid
    })
  ...
  defp call_stream_next(op, stream, :infinity, pid) do
    ...
    telemetry(:stream, op, %{duration: :os.system_time(:microsecond) - event_time}, %{
      stream_buffer_size: buffer_size,
      stream_chunk_size: :infinity,
      pid: pid
    })
  ...
  defp collect_result_refs(config, result, pid) when is_map_key(config.mon, pid),
    do: update_in(config, [:mon, pid], &scan_result_refs(result, &1))
  ...
  defp scan_result_refs(map, set) when is_map(map) and not is_struct(map),
    do:
      Enum.reduce(map, set, fn {key, val}, set ->
        scan_result_refs(val, scan_result_refs(key, set))
      end)

  defp scan_result_refs(list, set) when is_list(list),
    do: Enum.reduce(list, set, &scan_result_refs/2)

  defp scan_result_refs(tuple, set) when is_tuple(tuple),
    do: scan_result_refs(Tuple.to_list(tuple), set)
  ...
  defp scan_result_refs(%Stream{enum: enum}, set) when is_struct(enum, GitStream),
    do: scan_result_refs(enum, set)
  ...
  defp resolve_tree_entry({mode, type, oid, name}),
    do: %GitTreeEntry{oid: oid, name: name, mode: mode, type: type}

  defp resolve_index(index), do: %GitIndex{__ref__: index}

  defp resolve_diff_delta({{old_file, new_file, count, similarity}, hunks}) do
    %{
      old_file: resolve_diff_file(old_file),
      new_file: resolve_diff_file(new_file),
      count: count,
      similarity: similarity,
      hunks: Enum.map(hunks, &resolve_diff_hunk/1)
    }
  end
  ...
  defp resolve_diff_hunk({{header, old_start, old_lines, new_start, new_lines}, lines}) do
    %{
      ...
      new_start: new_start,
      new_lines: new_lines,
      lines: Enum.map(lines, &resolve_diff_line/1)
    }
  end
  ...
  defp fetch_tree_entries(%GitTree{} = tree, path, handle) do
    with {:ok, tree_entry} <- fetch_tree_entry(tree, {:path, path}, handle),
         {:ok, tree} <- fetch_target(tree_entry, :tree, handle),
         do: fetch_tree_entries(tree, handle)
  end
  ...
  defp zip_tree_entries_target(with_target, commit, {root_tree_entry, path_map}, handle) do
    {tree_entries_with_commit, path_map} =
      zip_tree_entries_target(with_target, commit, path_map, handle)

    {[{root_tree_entry, resolve_peel!(commit, with_target, handle)} | tree_entries_with_commit],
     path_map}
  end

  defp zip_tree_entries_target(with_target, commit, path_map, handle)
       when map_size(path_map) > 0 do
    path_map
    |> Enum.filter(fn {path, _entry} -> pathspec_match_commit(commit, [path], handle) end)
    |> Enum.reduce({[], path_map}, fn {path, entry}, {acc, path_map} ->
      {[{entry, resolve_peel!(commit, with_target, handle)} | acc], Map.delete(path_map, path)}
    end)
  end
  ...
  defp fetch_diff(obj1, obj2, handle, opts) do
    with {:ok, tree1} <- fetch_tree(obj1, handle),
         {:ok, tree2} <- fetch_tree(obj2, handle),
         do: fetch_diff(tree1, tree2, handle, opts)
  end
  ...
  defp walk_history(rev, handle, opts \\ []) do
    {sorting, opts} =
      Enum.split_with(opts, &(is_atom(&1) && String.starts_with?(to_string(&1), "sort")))
