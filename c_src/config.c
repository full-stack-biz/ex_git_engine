...
{
	git_engine_config *cfg = (git_engine_config *) cd;
	git_config_free(cfg->config);
}

ERL_NIF_TERM
git_engine_config_open(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
...
static ERL_NIF_TERM extract(git_engine_config **cfg, ErlNifBinary *bin, ErlNifEnv *env, const ERL_NIF_TERM argv[])
{
	if (!enif_get_resource(env, argv[0], git_engine_config_type, (void **) cfg))
		return enif_make_badarg(env);
...
ERL_NIF_TERM
git_engine_config_set_bool(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	git_engine_config *cfg;
	ErlNifBinary bin;
	int error, val;
...
ERL_NIF_TERM
git_engine_config_get_bool(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	git_engine_config *cfg;
	ErlNifBinary bin;
	int error, val;
...
ERL_NIF_TERM
git_engine_config_set_int(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	git_engine_config *cfg;
	ErlNifBinary bin;
	ErlNifSInt64 val;
...
ERL_NIF_TERM
git_engine_config_get_string(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	git_engine_config *cfg;
	...
	error = git_engine_string_to_bin(&result, buf.ptr);
	git_buf_free(&buf);
...
ERL_NIF_TERM
git_engine_config_set_string(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	git_engine_config *cfg;
	...
	if (!git_engine_terminate_binary(&val))
		return git_engine_oom(env);
