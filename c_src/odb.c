...
{
	git_engine_odb *odb = (git_engine_odb *)cd;
...
static ERL_NIF_TERM indexer_progress_to_map(ErlNifEnv *env, git_indexer_progress* progress)
{
	...
	if (enif_make_map_from_arrays(env, keys, values, 7, &map_term) < 0)
		return git_engine_error(env); // TODO

	return map_term;
}

ERL_NIF_TERM
git_engine_odb_hash(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	int error;
	...
	type = git_engine_object_atom2type(argv[0]);
	if (type == GIT_OBJ_BAD)
...
ERL_NIF_TERM
git_engine_odb_exists(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	git_engine_odb *odb;
	ErlNifBinary bin;
	git_oid oid;
	...
	if (!enif_get_resource(env, argv[0], git_engine_odb_type, (void **)&odb))
		return enif_make_badarg(env);
...
ERL_NIF_TERM
git_engine_odb_read(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	int error;
	ErlNifBinary bin;
	git_oid id;
	...
	return enif_make_tuple3(env, atoms.ok, git_engine_object_type2atom(type), enif_make_binary(env, &bin));
}

ERL_NIF_TERM
git_engine_odb_write(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	int error;
	git_otype type;
	git_oid oid;
...
ERL_NIF_TERM
git_engine_odb_write_pack(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	int error;
	git_engine_odb *odb;
	ErlNifBinary bin;
...
ERL_NIF_TERM
git_engine_odb_get_writepack(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	int error;
	git_engine_odb *odb;
	git_engine_odb_writepack *odb_writepack;
	...
	odb_writepack = enif_alloc_resource(git_engine_odb_writepack_type, sizeof(git_engine_odb_writepack));
...
ERL_NIF_TERM
git_engine_odb_writepack_append(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	int error;
	git_engine_odb_writepack *odb_writepack;
	ErlNifBinary bin;

	if (!enif_get_resource(env, argv[0], git_engine_odb_writepack_type, (void **)&odb_writepack))
		return enif_make_badarg(env);
...
ERL_NIF_TERM
git_engine_odb_writepack_commit(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
	int error;
	git_engine_odb_writepack *odb_writepack;
